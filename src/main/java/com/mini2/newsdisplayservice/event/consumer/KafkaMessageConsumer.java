...
//    private final StringRedisTemplate stringRedisTemplate;
    ...
    void handleFavoriteInfoEvent(FavoriteInfoEvent event, Acknowledgment ack) {
        ...
        // Redis Key: user:{userId}:bookmarks
        String redisKey = "user:" + userId + ":bookmarks";
...
//            log.info("Redis에 북마크 추가됨: Key={}, NewsId={}", redisKey, newsId);
//        } else if ("좋아요_취소".equalsIgnoreCase(eventType)) { // 대소문자 무시
//            stringRedisTemplate.opsForSet().remove(redisKey, newsId);
//            log.info("Redis에서 북마크 제거됨: Key={}, NewsId={}", redisKey, newsId);
//        } else {
